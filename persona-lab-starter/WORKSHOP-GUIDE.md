...
If persona-lab already exists in my namespace, it's fine to delete that repository and recreate it from this starter. Check the exact owner and repository name first, and only replace that repository. I understand this removes its existing code, issues, and pull requests.
...
"Personalize the Persona Lab company story" and three linked sub-issues so we can
...
2. Add fictional clients and teammates
   ...
   - Add fictional client logo marks with DiceBear's `initials` or `shapes` style, using each client name as the seed. Save the SVGs locally too.
...
3. Add fictional supporters and company footer
   ...
   - Generate simple synthetic logo marks for supporters with DiceBear's `initials` or `shapes` style, using each organization name as the seed. For example: `https://api.dicebear.com/10.x/shapes/svg?seed=Placeholder%20Partners&size=96`.
...
Coordinate them from this task. Have Brand identity share its chosen
name, tagline, and palette early, and send those decisions to the
other two tasks so the sections feel consistent. Include a commit
or PR link when another task needs the actual code changes.

When they're finished, review the three PRs for overlapping changes
and tell me the recommended merge order. Leave the PRs open for review.
```

This exercise uses separate Codex tasks with their own worktrees and PRs. The coordinating task passes decisions between them; code changes still need to be integrated through Git.

## Task 4 — Review the UI with subagents

Run this exercise after improving the frontend.

### 1. Create the issue

```text
Create a GitHub issue called "Make the site work better on mobile and easier to use".
...
```text
Work on <ISSUE_URL> using two subagents—one to review mobile layout
and visual consistency, and another to review accessibility.
Have them inspect the site in the browser and report what they find
without editing files. Then make the fixes yourself, verify them in
the browser, and open a PR linked to the issue. Leave it open for review.
```
